...
mainApp.controller('websocketControllerV2', ['$scope', '$location', 'WS', function($scope, $location, WS) {
    console.log($location.host() + ':' + $location.port());
    var port = ':' + $location.port();
    if ($location.port() == '') {
        port = '';
    }
    var address = 'ws://' + $location.host() + port + '/websocket-v2';
    $scope.wsin = '';
    $scope.wsout = [];
    $scope.active = true;
    ...
    $scope.begin = function() {
        ...
        $scope.buttonmsg = "Connecting to v2 stream...";
